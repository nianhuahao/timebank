...
// 创建用户
func (t *ServiceSetup) CreateUsers(c *gin.Context) {
	...
	if body.UserName == "" || body.Sex == "" || body.UserIdentification == "" || body.Birthday == "" || body.Address == "" || body.Postcode == "" || body.Ability == "" {
...
// 创建工作
func (t *ServiceSetup) CreateJobs(c *gin.Context) {
	respG := TB_Utils.Gin{C: c}
	body := new(JobPrice)
	...
	if body.JobName == "" || body.JobUnitCost <= 0 || body.DetailedDescription == "" {
...
// 创建组织
func (t *ServiceSetup) CreateOrgs(c *gin.Context) {
	respG := TB_Utils.Gin{C: c}
	...
	var bodyBytes [][]byte
	bodyBytes = append(bodyBytes, []byte(body.OrgID))
	bodyBytes = append(bodyBytes, []byte(body.OrgName))
