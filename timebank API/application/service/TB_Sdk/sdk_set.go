...
func CreateAndJoinChannel(info *SdkEnvInfo) error {
	...
	for _, org := range info.Orgs {
		...
		if err := org.OrgResMgmt.JoinChannel(info.ChannelID, resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithOrdererEndpoint("orderer.jxnu.com")); err != nil {
...
func createChannel(signIDs []msp.SigningIdentity, info *SdkEnvInfo) error {
	...
	if _, err := chMgmtClient.SaveChannel(req, resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithOrdererEndpoint("orderer.jxnu.com")); err != nil {
	...
	for i, org := range info.Orgs {
		...
		if _, err = org.OrgResMgmt.SaveChannel(req, resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithOrdererEndpoint("orderer.jxnu.com")); err != nil {
