...
func (t *BlockChainTimeBank) Init(stub shim.ChaincodeStubInterface) peer.Response {
	...
	jobunitcost := [3]float64{
		20,
		10,
		5,
	}
	jobdescribtion := [3]string{
		"做得很好吃",
		"通天带",
		"洗得干净",
	}
	for i := 0; i < 3; i++ {
		job := lib.JobPrice{
			JobID:               jobid[i],
			JobName:             jobname[i],
			JobUnitCost:         jobunitcost[i],
			DetailedDescription: jobdescribtion[i],
		}
		_ = utils.WriteLedger(job, stub, lib.JobPriceKey, []string{jobid[i]})
	}
